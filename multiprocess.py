#stable_baselines version: 2.10.2
...
def consistency_satisfaction_model_quick(candidates,preference_profiles,n,model_name):
    ...
    with Pool() as pool:
        parm_list = [
        (candidates,copy.deepcopy(preference_profiles),int(n/8),clf.copy())
        ]*8
...
def consistency_satisfaction_model_quick_process(candidates,preference_profiles,n,clf):
    print("start model")
    sys.stdout.flush()
    time1 = time.time()
    groups_pfs = []
    ...
    for i in range(2000):
        pp1 = random.choice(preference_profiles)
        pp1_Xs=np.array(labeling.get_Xs(candidates,[pp1]))
        preds_pp1=np.argmax(clf.predict(xgb.DMatrix(pp1_Xs)),axis = 1)[0]

        groups_pfs[preds_pp1].append(pp1)
    #for group in groups_pfs:
        #print(len(group))
    ...
    new_pps = []
    while profile_count<n:
        rand = random.randint(0,len(groups_pfs)-1)
        group_pfs = groups_pfs[rand]
        ...
        t1 = time.time()
        preds_pp1 = rand
        preds_newpp=np.argmax(clf.inplace_predict(new_pp_Xs),axis = 1)[0]
        t2 = time.time()
        pred_time+=t2-t1
        profile_count+=1
        #print(preds_pp1)
...
class CustomEnv(gym.Env):
  ...
  def step(self, action):
    ...
    if action == 0:
        ...
        self.us_pool_pfs.extend(copy.deepcopy(new_pfs))
        self.us_pool_Xs.extend(labeling.get_Xs(candidates,new_pfs))
        self.us_pool_labels.extend(new_labels)
    ...
    if action == 1:
        ...
        self.random_pool_pfs.extend(copy.deepcopy(new_pfs))
        self.random_pool_Xs.extend(labeling.get_Xs(candidates,new_pfs))
        self.random_pool_labels.extend(new_labels)
    ...
    if action ==2:
        n = 5
        new_pfs, new_winners, indexes = self.choose_neutrality_pfs(n)
    ...
    if action == 4:
        ...
        if len(self.consistency_labeled_pool_pfs1)>self.max_n:
            n = 1
        composite_set = copy.deepcopy(self.expert_labeled_pool_pfs+self.neutrality_labeled_pool_pfs)#+self.condorcet_labeled_pool_pfs)
        composite_ys = copy.deepcopy(self.expert_labeled_pool_labels+self.neutrality_labeled_pool_labels)#+self.condorcet_labeled_pool_labels)
    ...
    if action == 5:
        ...
        new_pfs,new_winners = generate_by_consistency1(candidates,copy.deepcopy(self.expert_labeled_pool_pfs+self.neutrality_labeled_pool_pfs),copy.deepcopy(self.expert_labeled_pool_labels+self.neutrality_labeled_pool_labels),self.condorcet_labeled_pool_pfs,self.condorcet_labeled_pool_labels,n)
    ...
    old_designer_accuracy = self.designer_accuracy
    ...
    done = (len(self.us_pool_pfs)+(len(self.random_pool_pfs)*0.8) + (len(self.neutrality_labeled_pool_pfs)*self.ratio)+ (len(self.condorcet_labeled_pool_pfs)*self.ratio) + (len(self.consistency_labeled_pool_pfs1)*self.ratio) + (len(self.consistency_labeled_pool_pfs2)*self.ratio))>=self.BUDGET
    ...
    self.reward = self.designer_accuracy- old_designer_accuracy
    print(self.reward)
    if done == True:
        """
        ...
        self.consistency_satisfaction = consistency_satisfaction_model_quick(candidates,preference_profiles_test,100000,model_name)
        ...
        self.reward += (self.condorcet_satisfaction+self.consistency_satisfaction)
        ...
        (read_dictionary['final_states']).append(tuple(self.state[self.state_length*(self.num_states-1):self.state_length*self.num_states]))
  ...
  def reset(self):
    ...
    self.random_pool_pfs = copy.deepcopy(self.pfs_warm_up)
    self.random_pool_Xs = labeling.get_Xs(candidates,self.pfs_warm_up)
    self.random_pool_labels = copy.deepcopy(labeling.get_labels(candidates,self.pfs_warm_up,self.voting_rule))
    ...
    self.designer_accuracy = 0
    
    self.reward = 0
    
    self.experience = [0]*self.state_length*self.num_states
    self.state = self.get_state(model_name)
    self.print_status()
  ...
  def loss(self,Xs, labels,model_name):
    ...
    preds = clf.predict(xgb.DMatrix(Xs))
    preds = np.array(preds)
    loss = cross_entropy_loss(labels,preds)/len(labels)*10
    #preds = np.argmax(preds,axis = 1)
  ...
  def get_state(self, model_name):
    l = []
    l.append(len(self.us_pool_pfs))
    l.append(len(self.random_pool_pfs))
   ...
   # l.append(len(self.neutrality_unlabeled_pool_pfs))
    ...
    """
    us_loss = self.loss(np.array(self.us_pool_Xs), np.array(self.us_pool_labels),model_name)
    random_loss = self.loss(np.array(self.random_pool_Xs), np.array(self.random_pool_labels),model_name)
    neutrality_loss = self.loss(np.array(self.neutrality_labeled_pool_Xs), np.array(self.neutrality_labeled_pool_labels),model_name)
    ...
    l.append(us_loss)
    l.append(random_loss)
    l.append(neutrality_loss)
    l.append(condorcet_loss)
    l.append(consistency_loss1)
    l.append(consistency_loss2)

    new_experience = self.experience[self.state_length:self.state_length*self.num_states]+l
...
if __name__=='__main__':
    ...
    params = {'objective':'multi:softprob', 'max_depth' :2,'n_estimators':3, 'num_class':len(candidates)}#WAS 10,5
    ...
    log_dir = "consistency_PPO_4candidates_5voters_50BUDGET_0.1_6actions_0/"
    import shutil
    ...
    #from stable_baselines.deepq.policies import MlpPolicy
    #model = DQN(MlpPolicy, env, gamma=1.0, learning_starts=20000, batch_size=20000,learning_rate=0.0000025, verbose=1)
    ...
    model = PPO2(MlpPolicy, env, policy_kwargs=policy_kwargs, learning_rate=0.00005,gamma = 1,verbose=1, nminibatches=2,noptepochs = 10,n_steps = 200)
    #model.set_env(env)
    model.learn(1000000,callback = call_back)
    #stats_path = os.path.join(log_dir, "vec_normalize.pkl")
    #env.save(stats_path)

    model.save("consistency_PPO_4candidates_5voters_50BUDGET_0.1_6actions_A")